...
def get_phone_info():
    while True:
        ...
        cc = input(mesgdcrt.CommandMessage(
            "Hududingiz kodini kiriting (+ belgisi yozmasdan): "))
        cc = format_phone(cc)
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.GeneralMessage("Raqam       : " + cc + " " + target)
    mesgdcrt.GeneralMessage("Yuborildi         : " + str(requested))
    mesgdcrt.GeneralMessage("Bordi   : " + str(success))
    mesgdcrt.GeneralMessage("Bormadi       : " + str(failed))
    mesgdcrt.WarningMessage(
        "This tool was made for fun and research purposes only")
    mesgdcrt.SuccessMessage("UzWhoamiBomber was created by UZWHOAMI")
...
def workernode(mode, cc, target, count, delay, max_threads):
    ...
    input(mesgdcrt.CommandMessage(
        "Ortga qaytib sozlamarni qayta to'g'irlash uchun [CTRL+Z] tugmalarini bosing, davom etish uchun [ENTER] tugmasini bosing."))
    ...
    if len(APIProvider.api_providers) == 0:
        ...
        input(mesgdcrt.CommandMessage("Chiqish uchun [ENTER] ni bosing."))
        bann_text()
        sys.exit()
    ...
    print("\n")
    mesgdcrt.SuccessMessage("Bomber tugatildi!")
    time.sleep(1.5)
    bann_text()
    sys.exit()
            ...
            while (choice not in avail_choice):
                ...
                for key, value in avail_choice.items():
                    print("[ {key} ] {value} BOMB".format(key=key,
                                                          value=value))
                print()
                choice = input(mesgdcrt.CommandMessage("Bu yerga kiriting : "))
