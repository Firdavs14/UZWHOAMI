...
pause() {
    read -n1 -r -p "Davom etish uchun [ENTER]..." key
}
banner() {
    clear
    echo -e "\e[1;31m"
    ...
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m  Created By \e[1;32mUZWHOAMI \e[0m"
    else
...
do
    ...
    echo " "
    echo "SMS  Bomber ni boshlash uchun 1 ni bosing"
    echo "CALL Bomber ni boshlash uchun 1 ni bosing"
    echo "MAIL Bomber ni boshlash uchun 3 ni bosing"
    echo " "
    echo "Chiqish uchun 5 ni bosing "
    read ch
    clear
    if [ $ch -eq 1 ];then
